...
function filter(){    
  ...
  ProductName = ProductList.getElementsByClassName("col-2");     
...
// Sign Up
function checkInfo(){
  var strFName = document.getElementById("ifristName").value
  var reName = /^[a-zA-Z][a-zA-Z ]+$/
  if(!reName.test(strFName)){
      alert("the Full name cannot left blank!")
      document.getElementById("ifristName").focus()
      return false
  }
  var strAddress = document.getElementById("address").value
  var reAddress = /^[a-zA-Z][a-zA-Z ][0-9]+$/
  if(!reAddress.test(strAddress)){
      alert("the Address cannot left blank!")
      document.getElementById("address").focus()
      return false
  }
  var strMail = document.getElementById("iEmail").value
  var reMail = /^[a-zA-Z]\w+[@]\w+[.]\w{3}([.]\w{2})?$/
  ...
  var selCountry = document.getElementById("iCountry")
  var i = selCountry.selectedIndex
  var iCountry = selCountry.options[i].value
  ...
  var showInfo = []
  showInfo.push("Register Infomation")
  showInfo.push("First Name: " + strFName)
  showInfo.push("Address: " + strAddress)
  showInfo.push("Country: " + iCountry)
  showInfo.push("Gender: " + sGender)
  showInfo.push("Email: " + strMail)
  showInfo.push("Phone: " + strPhone)
  
  alert(showInfo.join("\n"))
}
// Sign In
function checkLoginInfo(){
  var strLoginName = document.getElementById("inputName").value
  var strPassName = document.getElementById("inputPassword").value
  if(strLoginName == "aptech" && strPassName == 123){
      alert("Welcome COZY AIRCONDITIONERS, Sign in successfully")
  }else{
